...
def timer_decorator(func: Callable) -> Callable:
    ...
    def timer_wrapper(*args: int, **kwargs: Any) -> List[int]:
        start = time.time()
        func(*args, **kwargs)
        end = time.time()
        if int(end - start) > 0:
...
def _check_number_input(n: int, min_thresh: int, max_thresh: Optional[int] = None) -> bool:
    """
...
@timer_decorator
def fib_formula(n: int) -> List[int]:
    """
    :param n: calculate Fibonacci to the nth integer
    :type n:int
    :return: Fibonacci sequence as a list
