...
def scalar_multiply(matrix: map, n: float) -> List[List[float]]:
...
def transpose(matrix: List[List[int]], return_map: bool = True) -> map:
...
def _shape(matrix: List[List[int]]) -> List[int]:
    return list((len(matrix), len(matrix[0])))


def _verify_matrix_sizes(matrix_a: List[List[int]], matrix_b: List[List[int]]) -> Tuple[List[int], List[int]]:
