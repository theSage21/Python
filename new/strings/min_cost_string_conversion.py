...
'''
def compute_transform_tables(X: str, Y: str, cC: int, cR: int, cD: int, cI: int) -> Tuple[List[List[int]], List[Union[List[Union[int, str]], List[str]]]]:
	X = list(X)
	Y = list(Y)
	m = len(X)
	n = len(Y)
...
def assemble_transformation(ops: List[Union[List[Union[int, str]], List[str]]], i: int, j: int) -> Any:
	if i == 0 and j == 0:
		seq = []
		return seq
	else:
